...
pred=prediction(twitterTrain$Prediction,twitterTrain$Choice)
...
library(igraph)
edges_tmp=subset(pairs,score>=0.5 & A_ID<20 & B_ID<20)
edges=data.frame(edges_tmp$A_ID,edges_tmp$B_ID)
g = graph.data.frame(edges, TRUE, c(seq(0,19,1)))
V(g)$size =20*unique_users$score
plot(g, vertex.label=NA, edge.arrow.size=0.5, edge.color="gray24")
...
tapply(new_users$score, ClusterGroups, mean)
...
finalTree=rpart(cluster~.,data=finalTrain, method="class", cp=0.002)
prp(finalTree)
PredictCart=predict(finalTree, type="class")
table(finalTrain$cluster,PredictCart)
(64+143+56+85)/nrow(finalTrain)
finalTreeTest=predict(finalTree,newdata=finalTest, type="class")
table(finalTest$cluster,finalTreeTest)
(27+59+22+32)/nrow(finalTest)

library(randomForest)
finalTrain$cluster=as.factor(finalTrain$cluster)
finalTest$cluster=as.factor(finalTest$cluster)
twitterForest=randomForest(cluster~.,data=finalTrain,ntree=500,nodesize=25)
PredictForest=predict(twitterForest)
table(finalTrain$cluster,PredictForest)
(65+142+56+79)/nrow(finalTrain)
PredictForestTest=predict(twitterForest,newdata=finalTest)
table(finalTest$cluster,PredictForestTest)
(29+62+21+35)/nrow(finalTest)
